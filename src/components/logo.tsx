...
export type LogoProps = {
  /**
   * Tailwind classes to style the SVG. Use text-... to control color, and width/height classes for size.
   * Example: text-gray-900 w-28 h-auto
   */
  className?: string;
  /**
   * Convenience size prop. If a number, it's treated as pixel width.
   * Tailwind classes still take precedence when provided in className.
   */
  size?: LogoSize;
  /** Accessible title for screen readers. If omitted, the icon is aria-hidden. */
  title?: string;
} & React.SVGProps<SVGSVGElement>;

const sizeToWidthClass: Record<Exclude<LogoSize, number>, string> = {
  xs: "w-16",
  sm: "w-20",
  md: "w-28",
  lg: "w-32",
...
/**
 * Logo component (inline SVG) with Tailwind-friendly customization.
 * - Color via text-* classes (fill follows currentColor)
 * - Size via width classes (e.g., w-28) and h-auto, or the `size` prop
 */
export function Logo({
  ...
  const widthClass =
    typeof size === "number" ? undefined : sizeToWidthClass[size];
  const inlineStyle =
    typeof size === "number"
      ? { width: size, height: "auto", ...(rest.style || {}) }
      : rest.style;
  ...
  return (
    <svg
      ...
      focusable="false"
      className={["h-auto", widthClass, className].filter(Boolean).join(" ")}
      style={inlineStyle}
      {...rest}
    >
